  ...
  return (
    <div className="flex-1 flex flex-col p-3 space-y-3 overflow-y-auto pb-20">
      ...
      <div className="game-panel border-glow rounded-xl p-3 space-y-2">
        <h3 className="font-display text-sm text-foreground flex items-center gap-2">{happinessEmoji} Happiness</h3>
      ...
      {/* Housing */}
      <div className="game-panel border-glow rounded-xl p-3 space-y-2">
        <h3 className="font-display text-sm text-foreground flex items-center gap-2">🏠 Housing</h3>
        ...
        <p className="text-sm text-muted-foreground">Max houses = Town Hall level × 2. Upgrade Town Hall to build more.</p>
      </div>

      {/* Population Overview */}
      <div className="game-panel border-glow rounded-xl p-3 space-y-2">
        <h3 className="font-display text-sm text-foreground flex items-center gap-2">👥 Population</h3>
        <div className="flex items-center gap-3">
          <div className="flex-1 bg-muted rounded-full h-3 overflow-hidden">
        ...
        <div className="grid grid-cols-3 gap-3 text-center">
          <div className="bg-primary/10 rounded-lg p-1.5">
            ...
            <p className="text-sm font-bold text-primary">⚔️ {population.soldiers}</p>
          </div>
          <div className="bg-amber-500/10 rounded-lg p-1.5">
            <p className="text-sm text-muted-foreground">Workers</p>
            <p className="text-sm font-bold text-amber-500">🔨 {population.workers}</p>
          </div>
          <div className="bg-emerald-500/10 rounded-lg p-1.5">
            <p className="text-sm text-muted-foreground">Idle</p>
            <p className="text-sm font-bold text-emerald-500">🏠 {population.civilians}</p>
          </div>
        </div>
        <p className="text-sm text-muted-foreground">Army Cap: ⚔️ {population.armyCap} (from Barracks + workers)</p>
      </div>

      {/* Rations Control */}
      <div className="game-panel border-glow rounded-xl p-3 space-y-2">
        <h3 className="font-display text-sm text-foreground flex items-center gap-2">🍖 Rations</h3>
        <div className="grid grid-cols-3 gap-2.5">
          {(['scarce', 'normal', 'generous'] as RationsLevel[]).map(r => {
            ...
            return (
              ...
              >
                <p className="text-sm font-display font-bold">{info.label}</p>
                <p className="text-sm">{info.foodMultiplier}× food</p>
                <p className={`text-sm ${info.happinessBonus > 0 ? 'text-emerald-500' : info.happinessBonus < 0 ? 'text-destructive' : ''}`}>
      ...
      {/* Tax Control */}
      <div className="game-panel border-glow rounded-xl p-3 space-y-2">
        <h3 className="font-display text-sm text-foreground flex items-center gap-2">💰 Population Tax</h3>
        <div className="flex items-center gap-3">
          <input
            type="range"
            min={0} max={30} step={1}
          ...
          />
          <span className="text-sm font-bold text-foreground w-8 text-right">{popTaxRate}%</span>
        </div>
        <div className="flex justify-between text-sm">
        ...
        {popTaxRate > 10 && (
          <p className="text-sm text-destructive">⚠️ High taxes reduce happiness by -{(popTaxRate - 10) * 2}</p>
        )}
      </div>

      {/* Resources & Steel */}
      <div className="game-panel border-glow rounded-xl p-3 space-y-1.5">
        <h3 className="font-display text-sm text-foreground">📊 Resource Summary</h3>
        <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
          ...
          {steelProduction > 0 ? (
            ...
            <p className="text-sm text-muted-foreground col-span-2">Capture iron mines on the Map to produce steel</p>
          )}
        </div>
      </div>
      ...
      {totalTroops > 0 && (
        <div className="game-panel border-glow rounded-xl p-3 space-y-2">
          <h3 className="font-display text-sm text-foreground">⚔️ Army Upkeep</h3>
            ...
            {(['militia', 'archer', 'knight', 'cavalry', 'siege'] as TroopType[]).map(type => {
              ...
              return (
                <div key={type} className="flex items-center justify-between text-sm">
                  <span className="text-foreground flex items-center gap-2"><TroopIcon type={type} size={14} /> {info.name} x{count}</span>
      ...
      {/* Worker Assignments */}
      <div className="game-panel border-glow rounded-xl p-3 space-y-2">
        <h3 className="font-display text-sm text-foreground">🔨 Worker Assignments</h3>
        <p className="text-sm text-muted-foreground">Assign idle civilians ({population.civilians} available) to buildings for production bonuses</p>
            ...
            <div key={b.id} className="space-y-0.5">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <span className="text-sm text-foreground">{info.name} Lv.{b.level}</span>
                    <p className="text-sm text-muted-foreground">{current}/{max} workers</p>
                  </div>
                </div>
              ...
              {Object.keys(prod).length > 0 && (
                <div className="flex gap-3 text-sm text-muted-foreground ml-7">
        ...
        {buildings.filter(b => b.type !== 'empty' && BUILDING_INFO[b.type as Exclude<BuildingType, 'empty'>].workersPerLevel > 0).length === 0 && (
          <p className="text-sm text-muted-foreground text-center py-3">Build production buildings to assign workers</p>
        )}
      </div>

      {/* Civilian Cost */}
      <div className="game-panel rounded-xl p-3 space-y-1">
        <h3 className="font-display text-sm text-foreground">🏠 Civilian Costs</h3>
        <div className="flex justify-between text-sm">
          <span className="text-muted-foreground">Civilian food consumption</span>
          <span className="text-destructive">🌾-{popFoodCost}/min</span>
        </div>
        <p className="text-sm text-muted-foreground">Based on rations: {RATIONS_INFO[rations].description}</p>
      </div>
    </div>
  );
}
